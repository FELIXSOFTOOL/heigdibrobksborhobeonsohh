        ...
        if row:
            ...
            conn.execute(
                """INSERT OR IGNORE INTO daily_report
                   (number, user_id, username, report_date, payment_status)
                   VALUES (?,?,?,?,?)""",
                (number, uid, uname, today, "unpaid")
            )
            conn.execute(
                "UPDATE daily_report SET stood_at=?, payment_status='unpaid' WHERE user_id=? AND number=? AND report_date=?",
                (now.isoformat(), uid, number, today)
            )
...
async def cb_op_fell(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with db() as conn:
        ...
        if row:
            _n, _uid, _uname, _ = row
            conn.execute(
                "UPDATE daily_report SET deleted_from_report=1, payment_status='fell' WHERE user_id=? AND number=? AND report_date=?",
                (_uid, _n, date.today().isoformat())
            )
