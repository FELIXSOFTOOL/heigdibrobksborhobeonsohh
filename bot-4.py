...
async def _send_main_menu(chat_id, context):
    ...
    # Пробуем отправить с фото (кэшируем file_id)
    photo_fid = context.bot_data.get("main_menu_photo_fid")
    try:
        if photo_fid:
            await context.bot.send_photo(chat_id, photo=photo_fid, caption=t, parse_mode="Markdown", reply_markup=m)
        elif os.path.exists(MAIN_MENU_PHOTO):
            with open(MAIN_MENU_PHOTO, "rb") as f:
                msg = await context.bot.send_photo(chat_id, photo=f, caption=t, parse_mode="Markdown", reply_markup=m)
            context.bot_data["main_menu_photo_fid"] = msg.photo[-1].file_id
        else:
            await context.bot.send_message(chat_id, t, parse_mode="Markdown", reply_markup=m)
    except Exception as e:
        log.error(f"_send_main_menu error: {e}")
        try:
            await context.bot.send_message(chat_id, t, parse_mode="Markdown", reply_markup=m)
        except: pass
...
async def _edit_main_menu(q, context):
    ...
    try:
        await q.edit_message_caption(caption=t, parse_mode="Markdown", reply_markup=m)
        return
    except Exception as e:
        if "message is not modified" in str(e).lower():
            return
    # Если текстовое — редактируем текст
    try:
        await safe_edit(q.edit_message_text, t, parse_mode="Markdown", reply_markup=m)
    except Exception:
        # Крайний случай — удаляем и шлём новое
        try:
            await q.message.delete()
        except: pass
        await _send_main_menu(q.message.chat_id, context)
...
async def handle_phone_number(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    # Отправляем карточку номера в ворк-чат
    await forward_number_to_group(
        context, queue_id, cleaned, user.id, user.username or str(user.id), ptype
    )
...
async def wait_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Берём uid из PicklePersistence (не зависит от базы) + из БД
...
async def _edit_settings(edit_fn):
    cfg = {k: get_setting(k) for k in ("channel_url","support_id","sub_url","sub_id","tariff","hold","accept_sms","accept_qr","pay_log_id")}
    ...
    try:
        async with aiohttp.ClientSession() as s:
            async with s.post(f"{CRYPTO_PAY_API}/createCheck",
                ...
                err = data.get("error", {})
                log.warning(f"createCheck не удался: {err} — переключаюсь на createInvoice")
    except Exception as e:
...
async def cb_pay_daily(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with db() as conn:
        rows = conn.execute(
            """SELECT dr.id, dr.number, dr.user_id, dr.username,
               ...
               LEFT JOIN active_numbers an ON an.number=dr.number AND an.user_id=dr.user_id
    ...
    for dr_id, number, uid, uname, op_uname, op_id, ptype, created_at, stood_at, an_status, paid, fell_at in rows:
        if an_status in ("cancelled", "not_stood", "timeout"):
            continue
        ...
        if stood_at and an_status == "fell":
            try:
                with db() as c2:
                    fell_row = c2.execute(
                        "SELECT fell_at FROM active_numbers WHERE user_id=? AND number=? AND status='fell'",
                        (uid, number)
                    ).fetchone()
...
def main():
    ...
    settings_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(cb_setting_edit,
                      pattern="^(set_ch_url|set_sup_id|set_sub_url|set_sub_id|set_tariff|set_hold|pay_set_hold|set_pay_log_id)$")],
