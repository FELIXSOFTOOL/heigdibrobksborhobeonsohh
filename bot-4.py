...
async def crypto_create_check(amount: float, currency: str = "USDT") -> dict:
    """Создаёт CryptoPay чек (ваучер) для возврата средств покупателю."""
    headers = {"Crypto-Pay-API-Token": CRYPTO_PAY_TOKEN}
    payload  = {"asset": currency, "amount": str(amount)}
    try:
        async with aiohttp.ClientSession() as s:
            async with s.post(f"{CRYPTO_PAY_API}/createCheck", json=payload, headers=headers) as r:
                data = await r.json()
                return data.get("result", {}) if data.get("ok") else {}
    except Exception as e:
        log.error(f"CryptoPay createCheck error: {e}")
        return {}
...
async def cb_op_not_stood(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with db() as conn:
        row = conn.execute(
            "SELECT number, user_id, username, phone_type, op_id, status FROM active_numbers WHERE queue_id=?",
            (queue_id,)
        ).fetchone()
        conn.execute("UPDATE active_numbers SET status='disputed' WHERE queue_id=?", (queue_id,))
    ...
    if row:
        number, drop_uid, uname, phone_type, buyer_uid, prev_status = row
        ...
        is_buyer_ns   = (q.message and q.message.chat_id > 0)
        ns_text = (
