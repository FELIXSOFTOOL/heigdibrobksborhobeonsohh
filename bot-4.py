...
async def cmd_pay(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            ...
            f"Теперь он может вызывать /paymax сам.",
            parse_mode="Markdown"
        )
    except Exception as e:
        await update.message.reply_text(
            f"✅ Роль выдана, но не смог отправить панель: {e}"
        )
...
async def cmd_paymax(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin(uid) and context.args:
        ...
        try:
            ...
            await update.message.reply_text(f"✅ Роль выдана, но не смог отправить панель: {e}")
        return
