...
async def _refund_buyer_prepay(context, queue_id: int, number: str, reason: str = "Продавец не скинул код"):
    """
    Если номер был взят покупателем с предоплатой — возвращаем деньги через CryptoPay чек.
    """
    with db() as conn:
        pp = conn.execute(
    ...
    if not pp:
        return  # Предоплаты не было — ничего не делаем

    pp_id, buyer_uid, amount = pp

    # Создаём чек
    check = await crypto_create_check(amount, "USDT")
    check_url = check.get("bot_check_url") or check.get("check_url") or ""

    # Помечаем предоплату как использованную (чек отправлен — следующий номер платный)
    with db() as conn:
        conn.execute(
            "UPDATE buyer_prepay_pending SET status='consumed' WHERE id=?", (pp_id,)
        )

    # Отправляем покупателю уведомление с чеком
    try:
        check_line = f"\n\n🧾 [Получить средства]({check_url})" if check_url else \
                     "\n\n⚠️ Не удалось создать чек. Обратитесь к администратору."
        await context.bot.send_message(
            buyer_uid,
            f"💸 *Ваши деньги* ✅\n\n"
            f"{reason}, поэтому за один номер возвращается "
            f"`{amount:.2f} USDT`{check_line}",
            parse_mode="Markdown",
...
async def cb_op_req_sms(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with db() as conn:
        row = conn.execute(
            "SELECT number, user_id, username, chat_id FROM active_numbers WHERE queue_id=?", (queue_id,)
        ).fetchone()
    if not row:
        await q.answer("❌ Не найдено", show_alert=True); return

    number, uid, uname, chat_id = row
...
async def handle_qr_photo_from_op(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    try:
        send_kw = {
            ...
            "parse_mode": "Markdown",
            "reply_markup": InlineKeyboardMarkup([[
                InlineKeyboardButton("❌ Отменить / Не встал", callback_data=f"buy_cancel_ask_{queue_id}")
                if is_buyer_qr_sent else
...
async def _send_buyer_panel(bot, buyer_uid: int):
    ...
    bal_str      = f"\n💰 Баланс: *{bal:.4f}* USDT" if prepay_on else ""
...
def _buyer_min_topup() -> float:
    """Минимальное пополнение = макс.тариф + 0.15 USDT (на комиссию CryptoPay)."""
    ...
    COMMISSION = 0.15
    calculated = round(max_tariff + COMMISSION, 2)
    manual = _safe_float(get_setting("buyer_min_topup"), 0.0)
    if manual > 0:
        return max(manual, calculated)
    return calculated
...
async def _buy_take_number(update: Update, context: ContextTypes.DEFAULT_TYPE, ptype: str):
    ...
    if _buyer_prepay_on(buyer_uid):
        ...
        if prepay_amount > 0:
            # 1. Сначала проверяем баланс покупателя
            balance = _buyer_balance(buyer_uid)
            if round(balance, 4) >= round(prepay_amount, 4):
                _buyer_balance_deduct(buyer_uid, prepay_amount)
                # Снимаем с баланса — продолжаем выдачу номера без инвойса
            else:
                # 2. Проверяем refunded-кредит (номер не встал ранее)
                with db() as conn:
                ...
                if credit_row:
                    ...
                    # 3. Нет баланса и кредита — создаём инвойс
                    status_filter = "wait_sms" if ptype == "sms" else "wait_qr"
                    with db() as conn:
                        row = conn.execute(
                            "SELECT q.id FROM queue q WHERE q.status=? AND q.deleted=0 "
                            "ORDER BY q.loaded_at ASC LIMIT 1",
                            (status_filter,)
                        ).fetchone()
                    if not row:
                        ...
                        return
                    reserved_queue_id = row[0]
                    with db() as conn:
                        conn.execute("UPDATE queue SET status='taken' WHERE id=?", (reserved_queue_id,))
                    invoice = await crypto_create_invoice(
                        prepay_amount, "USDT",
                        f"Предоплата скупа {'СМС' if ptype=='sms' else 'QR'}"
                    )
                    if not invoice:
                        status_back = "wait_sms" if ptype == "sms" else "wait_qr"
                        with db() as conn:
                            conn.execute("UPDATE queue SET status=? WHERE id=?", (status_back, reserved_queue_id))
                        await q.answer("❌ Ошибка создания счёта.", show_alert=True)
                        return
                    invoice_id = invoice.get("invoice_id")
                    pay_url    = invoice.get("bot_invoice_url") or invoice.get("pay_url")
                    with db() as conn:
                        conn.execute(
                            "INSERT INTO buyer_prepay_pending "
                            "(buyer_uid, queue_id, ptype, invoice_id, amount, status, created_at) "
                            "VALUES (?,?,?,?,?,?,?)",
                            (buyer_uid, reserved_queue_id, ptype, invoice_id, prepay_amount,
                             "pending", now_msk().isoformat())
                        )
                    type_label_ru = "💬 СМС" if ptype == "sms" else "📷 QR"
                    kb = InlineKeyboardMarkup([
                        [InlineKeyboardButton(f"💳 Оплатить {prepay_amount:.2f} USDT", url=pay_url)],
...
async def cb_buy_dispute_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with db() as conn:
        an = conn.execute(
            "SELECT number, ptype, phone_type, status FROM active_numbers WHERE queue_id=? AND op_id=?",
            (queue_id, buyer_uid)
        ).fetchone()
    if not an:
        await q.answer("❌ Не найдено.", show_alert=True); return
...
async def global_buyer_photo_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    # Отправляем в dispute-канал
    dispute_chan = get_setting("dispute_channel_id")
    prepay_str   = f"\n💳 Предоплата: `{prepay_amt:.2f}` USDT" if prepay_amt else ""
    caption = (
        f"🚫 *Диспут #{dispute_id}*\n\n"
        f"Покупатель: {buyer_tag}\n"
        f"Номер: `{number}`\n"
        f"Метод: {type_label}{prepay_str}\n\n"
        f"📸 Скриншот от покупателя выше."
    )
    kb_d = InlineKeyboardMarkup([
        [InlineKeyboardButton("✅ Подтвердить (не встал)", callback_data=f"adm_dispute_ok_{dispute_id}")],
        [InlineKeyboardButton("❌ Отклонить (встал)",      callback_data=f"adm_dispute_rej_{dispute_id}")],
    ])
    ...
    if dispute_chan:
        try:
            ...
            msg = await context.bot.send_photo(
                _parse_chan_id(dispute_chan), photo,
                caption=caption, parse_mode="Markdown", reply_markup=kb_d
            )
            chan_msg_id = msg.message_id
            with db() as conn:
                conn.execute(
                    "UPDATE buyer_disputes SET channel_msg_id=? WHERE id=?",
                    (chan_msg_id, dispute_id)
                )
        except Exception as e:
            log.error(f"Не смог отправить диспут в канал: {e}")

    await update.message.reply_text(
        f"✅ *Жалоба #{dispute_id} отправлена администратору.*\n\n"
        f"{'Ожидайте решения.' if chan_msg_id else '⚠️ Канал диспутов не настроен. Свяжитесь с администратором.'}",
        parse_mode="Markdown"
    )
    return True

    # Проверяем что это реально карточка этого покупателя
    with db() as conn:
        an = conn.execute(
            "SELECT number, ptype, op_id, status FROM active_numbers WHERE queue_id=?",
            (queue_id,)
        ).fetchone()
    if not an or an[2] != buyer_uid:
        await q.answer("❌ Не найдено.", show_alert=True)
        return
    number, ptype, _, an_status = an
    if an_status in ("disputed", "not_stood", "cancelled"):
        await q.answer("⚠️ Диспут уже открыт или номер уже закрыт.", show_alert=True)
        return

    # Находим prepay_id если есть
    with db() as conn:
        pp = conn.execute(
            "SELECT id, amount FROM buyer_prepay_pending "
    ...
    # Создаём запись диспута
    with db() as conn:
        conn.execute(
            "INSERT INTO buyer_disputes "
            "(queue_id, buyer_uid, number, ptype, prepay_id, status, created_at) "
            "VALUES (?,?,?,?,?,?,?)",
            (queue_id, buyer_uid, number, ptype, prepay_id, "open", now_msk().isoformat())
        )
        dispute_id = conn.execute("SELECT last_insert_rowid()").fetchone()[0]
        conn.execute(
            "UPDATE active_numbers SET status='disputed' WHERE queue_id=?", (queue_id,)
        )

    # Отправляем в dispute-канал
    dispute_chan = get_setting("dispute_channel_id")
    buyer_tag = f"@{esc_md(q.from_user.username)}" if q.from_user.username else f"ID `{buyer_uid}`"
    type_label = "💬 СМС" if ptype == "sms" else "📷 QR"
    prepay_str = f"\n💳 Предоплата: `{prepay_amt:.2f}` USDT" if prepay_amt else ""
    ...
    dispute_text = (
        ...
        f"Номер: `{number}`\n"
        f"Метод: {type_label}{prepay_str}\n\n"
        f"Покупатель утверждает что номер не встал.\n"
        f"Запросите скриншот подтверждения."
    )
    ...
    if dispute_chan:
        try:
            msg = await context.bot.send_message(
                _parse_chan_id(dispute_chan), dispute_text,
                parse_mode="Markdown", reply_markup=kb_dispute
            )
            chan_msg_id = msg.message_id
            with db() as conn:
                conn.execute(
            ...
            log.error(f"Диспут в канал не отправлен ({dispute_chan}): {e}")
            for adm in ADMIN_IDS:
                try:
                    await context.bot.send_message(
                        adm,
                        f"⚠️ *Диспут #{dispute_id} не отправлен в канал*\n"
                        f"Канал: `{dispute_chan}`\n"
                        f"Ошибка: `{e}`\n\n"
                        f"Убедись что бот добавлен в канал как *администратор* с правом отправки.",
                        parse_mode="Markdown"
                    )
                except: pass
    ...
    # Уведомляем покупателя
    await q.edit_message_text(
        f"🚫 *Номер не встал* — заявка отправлена\n\n"
        f"Номер: `{number}`\n\n"
        f"⏳ Ожидайте решения администратора.\n"
        f"{'✅ Диспут отправлен в канал.' if chan_msg_id else '⚠️ Канал диспутов не настроен, свяжитесь с администратором.'}",
        parse_mode="Markdown"
    )
...
async def cb_adm_dispute_ok(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Уведомляем покупателя
    try:
        prepay_note = "\n\n✅ Предоплата зачтена — следующий номер без оплаты." if prepay_id else ""
        await context.bot.send_message(
            buyer_uid,
            f"✅ *Диспут #{dispute_id} подтверждён*\n\n"
            f"Номер `{number}` не встал — подтверждено администратором.{prepay_note}",
            parse_mode="Markdown",
            reply_markup=_buyer_panel_markup(q.from_user.id)
        )
    except Exception as e:
    ...
    await safe_edit(
        ...
        f"Покупатель: {buyer_tag}\n"
        f"Номер: `{number}`\n"
        f"{'💳 Кредит предоплаты возвращён.' if prepay_id else 'Предоплаты не было.'}",
        parse_mode="Markdown"
    )
...
async def _do_req_pass(context, q, queue_id: int, is_repeat: bool = False):
    ...
    with db() as conn:
        row = conn.execute(
            "SELECT number, user_id, username, op_username, phone_type FROM active_numbers WHERE queue_id=?",
            (queue_id,)
        ).fetchone()
    if not row:
        await q.answer("❌ Не найдено", show_alert=True); return
...
async def handle_pass_code(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    with db() as conn:
        row = conn.execute(
            "SELECT number, username, chat_id, topic_id, group_msg_id, op_username FROM active_numbers WHERE queue_id=?",
            (queue_id,)
        ).fetchone()
    ...
    number, uname, chat_id, topic_id, group_msg_id, op_uname = row
    uname_safe = esc_md(uname) if uname else str(uid)
    op_safe    = esc_md(op_uname) if op_uname else "—"

    await update.message.reply_text("✅ *Пароль отправлен оператору.*", parse_mode="Markdown")

    # Отправляем пароль оператору в группу
    kb = InlineKeyboardMarkup([
        [
            InlineKeyboardButton("🟢 Встал",    callback_data=f"op_stood_{queue_id}"),
            InlineKeyboardButton("🔴 Не Встал", callback_data=f"op_not_stood_{queue_id}"),
...
async def cb_op_fell(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if stood_at_str:
        try:
            ...
            if total_secs < hold_min * 60:
                ...
                # До холда → cancelled, выплаты нет
                with db() as _c:
                    _c.execute(
                ...
                # После холда → оставляем payment_status='unpaid', дроп должен получить выплату
                # active_numbers.status='fell' остаётся для статистики
        except:
            duration = "—"
            fell_label = "💥 *Номер слетел*"
    ...
    try:
        if stood_at_str and total_secs < hold_min * 60:
...
async def handle_sms_code(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    with db() as conn:
        ...
        # Если номер уже отменён/истёк — не обрабатываем код
        if row and row[5] in ("cancelled", "not_stood", "stood", "paid", "disputed"):
...
async def cb_adm_buyer_reset_cfg(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await cb_adm_buyer_edit(update, context)
...
def main():
        ...
        if update.effective_chat.type == "private":
            ...
            if is_admin(update.effective_user.id):
                ...
                if context.user_data.get("wait_dispute_channel_id"):
                    ...
                    await update.message.reply_text(f"✅ Канал диспутов: `{raw}`", parse_mode="Markdown")
                    return
    ...
    async def global_photo_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if context.user_data.get("dispute_qid"):
            await global_buyer_photo_handler(update, context)
            return
        # QR от дропа?
        await handle_qr_photo_from_op(update, context)
