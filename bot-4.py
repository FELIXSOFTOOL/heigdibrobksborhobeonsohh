...
async def cb_op_fell(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with db() as conn:
        ...
        conn.execute("UPDATE active_numbers SET status='fell', sms_code=? WHERE queue_id=?",
                     (now_msk().isoformat(), queue_id))  # sms_code используем как fell_at временно
...
async def cb_pay_daily(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for dr_id, number, uid, uname, op_uname, op_id, ptype, created_at, stood_at, an_status, paid in rows:
        ...
        # Простой — от stood_at до fell_at (хранится в sms_code)
        duration = "—"
        if stood_at and an_status == "fell":
            try:
                with db() as c2:
                    fell_row = c2.execute(
                        "SELECT sms_code FROM active_numbers WHERE user_id=? AND number=? AND status='fell'",
                        (uid, number)
                    ).fetchone()
                if fell_row and fell_row[0]:
                    s_dt = datetime.fromisoformat(stood_at)
                    e_dt = datetime.fromisoformat(fell_row[0])
                    diff = e_dt - s_dt
