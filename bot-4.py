...
# ═══════════════════════════════════════════════════════════
BOT_TOKEN         = "8675546625:AAGlvd4hiNKD8BH96YhLv-llZrI0tZDfFU0"
ADMIN_IDS         = [914120031, 8327037103]
CRYPTO_PAY_TOKEN  = "527750:AA4uKpQ01CRrOAS3cC3eK9TTrzCmxpi13oi"
...
async def cmd_setmax(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with db() as conn:
        conn.execute(
            "INSERT OR IGNORE INTO work_chats (chat_id, topic_id, added_by, added_at) VALUES (?,?,?,?)",
            (chat_id, topic_id, update.effective_user.id, now)
        )
    await update.message.reply_text(
        f"✅ Чат разрешён для ворка.\n`chat_id={chat_id}`, `topic={topic_id}`",
        parse_mode="Markdown"
    )
...
async def forward_number_to_group(context, queue_id: int, number: str,
    ...
    text = (
        ...
        f"Метод: {type_label}\n"
        f"Номер: `{number}`\n"
        f"Дроп: @{username or user_id}\n"
        f"Оператор: @{username or '—'} | ID: `{user_id}`"
    )
    ...
    if phone_type == "sms":
        ...
        kb = InlineKeyboardMarkup([[
            InlineKeyboardButton("📷 Показать QR",    callback_data=f"op_req_qr_{queue_id}"),
            InlineKeyboardButton("❌ Отменить номер", callback_data=f"op_cancel_{queue_id}"),
        ]])
...
async def _notify_pos3_if_needed(context, skip_queue_id: int = None):
    """Проверяет кто сейчас 3-й в очереди и отправляет ему уведомление если ещё не отправляли."""
    try:
        with db() as conn:
            rows = conn.execute(
        ...
        if len(rows) < 3:
            return
        q3_id, q3_uid, q3_num = rows[2]  # индекс 2 = позиция 3
        if skip_queue_id and q3_id == skip_queue_id:
            return
        # Проверяем не отправляли ли уже
        job_name = f"confirm_timeout_{q3_id}"
        existing = context.job_queue.get_jobs_by_name(job_name)
        if existing:
            return  # уже отправлено, таймер бежит
        ...
        log.info(f"✅ Уведомление pos3 → uid={q3_uid} qid={q3_id}")
    except Exception as e:
...
async def cb_op_req_sms(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        sent = await context.bot.send_message(
            ...
            f"Ответьте на это сообщение с кодом ниже 👇",
            parse_mode="Markdown"
        )
        with db() as conn:
...
async def cb_op_repeat_sms(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with db() as conn:
        row = conn.execute(
            "SELECT number, user_id FROM active_numbers WHERE queue_id=?", (queue_id,)
        ).fetchone()
    if not row:
        await q.answer("❌ Не найдено", show_alert=True); return
    ...
    try:
        sent = await context.bot.send_message(
            ...
            f"*Ответьте на это сообщение* с кодом 👇",
            parse_mode="Markdown"
        )
        context.bot_data[f"wait_sms_{uid}"] = queue_id
        context.bot_data[f"wait_sms_msg_{uid}"] = sent.message_id
        await q.answer("✅ Запрос отправлен дропу")
    except Exception as e:
...
async def handle_qr_photo_from_op(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    """Фото QR от оператора в группе — пересылаем дропу с кнопкой Отсканировал."""
    ...
    try:
        send_kw = {
            "chat_id": chat_id,
            "text": f"✅ *QR отправлен дропу* @{uname or uid}",
            "parse_mode": "Markdown",
            "reply_markup": InlineKeyboardMarkup([[
...
async def cb_drop_qr_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if row:
        ...
        try:
            send_kw = {"chat_id": chat_id, "text": f"❌ *Дроп @{uname or uid} отменил номер* `{number}`", "parse_mode": "Markdown"}
            if topic_id:
                send_kw["message_thread_id"] = topic_id
            await context.bot.send_message(**send_kw)
        except: pass
...
async def cmd_get_number(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "номер смс" in raw or "номер сms" in raw:
        ptype_filter = "sms"
    ...
    with db() as conn:
        ...
        if not row:
            slot_key = f"pending_slot_{chat_id}_{topic_id}"
            context.bot_data[slot_key] = {
                "statuses": [status_filter],
                "label":    ptype_filter,
                "op_id":    op.id,
                "op_name":  op.username or str(op.id),
            }
            await update.message.reply_text(
                f"⏳ Нет {'СМС' if ptype_filter=='sms' else 'QR'} номеров. Жду..."
            )
            return
...
async def _send_number_card_to_group(context, queue_id, number, user_id, uname, ptype,
    ...
    op_str = f"@{op_username}" if op_username else "—"
    ...
    with db() as conn:
        ...
        if existing:
            conn.execute(
                "UPDATE active_numbers SET chat_id=?, topic_id=?, group_msg_id=?, status='waiting' WHERE queue_id=?",
                (chat_id, topic_id, msg_id, queue_id)
            )
        else:
            conn.execute(
                """INSERT INTO active_numbers
...
async def cb_op_stood(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with db() as conn:
        ...
        # Убираем из очереди (встал = не считается в очереди)
        conn.execute("UPDATE queue SET status='stood', stood_at=?, deleted=1 WHERE id=?",
                     (now.isoformat(), queue_id))
        if row:
            number, uid, uname, op_uname, op_id_val = row
            conn.execute(
                "UPDATE daily_report SET stood_at=?, payment_status='unpaid' WHERE user_id=? AND number=? AND report_date=?",
                (now.isoformat(), uid, number, date.today().isoformat())
            )
    ...
    number, uid, uname, op_uname, op_id_val = row
    op_str    = f"@{op_uname}" if op_uname else "—"
    ...
    # Обновляем карточку в группе — новый формат + кнопка Слетел
    stood_text = (
        f"⚡ *Номер встал* ⚡\n\n"
        f"Номер: `{number}`\n"
        f"Время: {now_str}\n"
    ...
    try:
        msg = await q.edit_message_text(stood_text, parse_mode="Markdown", reply_markup=kb)
    except Exception as e:
...
async def cb_op_fell(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with db() as conn:
        row = conn.execute(
            "SELECT number, user_id, username, stood_at FROM active_numbers WHERE queue_id=?",
            (queue_id,)
        ).fetchone()
        conn.execute("UPDATE active_numbers SET status='fell' WHERE queue_id=?", (queue_id,))
        conn.execute("UPDATE queue SET status='fell', deleted=1 WHERE id=?", (queue_id,))

    if not row:
        await q.answer("❌ Не найдено", show_alert=True); return

    number, uid, uname, stood_at_str = row
...
async def handle_sms_code(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    queue_id = context.bot_data.get(f"wait_sms_{uid}")
    if not queue_id:
        return False

    # Проверяем что это ОТВЕТ на сообщение бота с запросом кода
    expected_msg_id = context.bot_data.get(f"wait_sms_msg_{uid}")
    if expected_msg_id:
        reply = update.message.reply_to_message
        if not reply or reply.message_id != expected_msg_id:
            # Не ответ на нужное сообщение — игнорируем
            return False
    ...
    if not code.isdigit() or len(code) != 6:
        ...
        context.bot_data[f"wait_sms_{uid}"] = queue_id
        if expected_msg_id:
            context.bot_data[f"wait_sms_msg_{uid}"] = expected_msg_id
        return True
    with db() as conn:
        row = conn.execute(
...
async def _timeout_confirm(context):
    ...
    with db() as conn:
        # Проверяем что ещё не подтвердили
        row = conn.execute(
            "SELECT status FROM active_numbers WHERE queue_id=?", (queue_id,)
        ).fetchone()
        if row and row[0] == "confirmed":
            return  # уже подтвердили

        conn.execute("UPDATE active_numbers SET status='cancelled' WHERE queue_id=?", (queue_id,))
        conn.execute("UPDATE queue SET deleted=1 WHERE id=?", (queue_id,))

    # Удаляем сообщение с подтверждением у дропа
...
async def cb_user_support(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.answer()
    uid = q.from_user.id
    context.bot_data[f"wait_support_{uid}"] = True
...
async def handle_support_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    uid = update.effective_user.id
    if not context.bot_data.get(f"wait_support_{uid}"):
        return False
    ...
    kb = InlineKeyboardMarkup([[
        InlineKeyboardButton("💬 Ответить", url=reply_url),
    ]])
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await _send_main_menu(update.message.reply_text, context)
...
async def cb_check_sub(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await check_sub(q.from_user.id, context):
        ...
        await _send_main_menu(q.message.reply_text, context)
    else:
...
async def cb_back_main(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    await _edit_main_menu(q.edit_message_text)

async def _main_menu_data():
    bot_on = get_setting("bot_status") == "on"
    cnt    = queue_count()
    status = "Статус: ✅ РАБОТАЕМ" if bot_on else "Статус: ⛔ НЕ РАБОТАЕМ"
    text   = f"⚡ *Главное меню* ⚡\n\n⏳ Очередь: *{cnt}* номеров\n{status}"
...
async def cb_phone_type(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Пользователь выбрал СМС или QR — ставим флаг ожидания номера."""
    q = update.callback_query
    await q.answer()
    uid   = q.from_user.id
    ptype = "sms" if q.data == "phone_type_sms" else "qr"

    # Флаг: ждём номер от этого юзера
...
async def handle_phone_number(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if not digits.isdigit() or len(digits) < 7:
        ...
        # Возвращаем флаг — дать ещё попытку
        context.bot_data[f"wait_phone_{uid}"] = ptype
        return True
    ...
    # Проверка дубля — номер уже в очереди?
    with db() as conn:
    ...
    with db() as conn:
        cursor = conn.execute(
            "INSERT INTO queue (number, status, user_id, operator, loaded_at) VALUES (?,?,?,?,?)",
            (cleaned, status, user.id, user.username or str(user.id), now.isoformat())
        )
        queue_id = cursor.lastrowid
        conn.execute(
            """INSERT INTO daily_report
               (number, user_id, username, operator, stood_at, report_date, payment_status)
               VALUES (?,?,?,?,?,?,?)""",
            (cleaned, user.id, user.username, user.username or str(user.id),
             now.isoformat(), date.today().isoformat(), "unpaid")
        )

    kb = [[InlineKeyboardButton("🏠 Главное меню", callback_data="back_main")]]
    await update.message.reply_text(
        "✅ *Номер принят. Жди когда админ возьмёт.*",
        parse_mode="Markdown",
        reply_markup=InlineKeyboardMarkup(kb)
    )
    ...
    # Проверяем pending слот
    chats = get_all_work_chats()
    for (wc_chat_id, wc_topic_id) in chats:
        slot_key = f"pending_slot_{wc_chat_id}_{wc_topic_id}"
        slot = context.bot_data.get(slot_key)
        if not slot:
            continue
        # Проверяем что тип принимается
        if status == "wait_sms" and get_setting("accept_sms") != "on":
            continue
        if status == "wait_qr" and get_setting("accept_qr") != "on":
            continue
        if status in slot["statuses"]:
            context.bot_data.pop(slot_key, None)
            with db() as conn:
                conn.execute("UPDATE queue SET status='taken' WHERE id=?", (queue_id,))
            await _send_number_card_to_group(
                context, queue_id, cleaned, user.id,
                user.username or str(user.id), ptype,
                wc_chat_id, wc_topic_id,
                op_username=slot.get("op_name"),
                op_id=slot.get("op_id")
            )
            try:
                await context.bot.send_message(
                    user.id,
                    f"🟩 Ваш номер `{cleaned}` взят.\n\nКод придёт в течение 1 минуты.",
                    parse_mode="Markdown"
                )
            except: pass
            break
    ...
    # Уведомляем админов
    if get_setting("notifications") == "on":
        type_label = "💬 СМС" if ptype == "sms" else "📷 QR"
...
async def cb_user_history(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    uid = q.from_user.id
    with db() as conn:
        # Только встоявшие номера
        rows = conn.execute(
            """SELECT an.number, an.stood_at, dr.paid
               FROM active_numbers an
               JOIN daily_report dr ON dr.user_id=an.user_id AND dr.number=an.number
               WHERE an.user_id=? AND an.status='stood'
               ORDER BY an.stood_at DESC LIMIT 30""",
            (uid,)
        ).fetchall()
        ...
        pending = conn.execute(
            """SELECT COUNT(*) FROM active_numbers an
               JOIN daily_report dr ON dr.user_id=an.user_id AND dr.number=an.number
               WHERE an.user_id=? AND an.status='stood' AND (dr.paid IS NULL OR dr.paid=0)""",
            (uid,)
        ).fetchone()[0]
...
async def _admin_data():
    ...
    text = (
        ...
        "Команды\n`/ban @user // ID`\n`/unban @user // ID`\n`/number`\n\n"
        f"Статус: {status_txt}\n"
        f"Уведомления: {notif_txt}"
    )
    kb = [
        [InlineKeyboardButton(f"Статус: {status_txt}", callback_data="adm_toggle_status")],
        [InlineKeyboardButton(f"Уведомления: {notif_txt}", callback_data="adm_toggle_notif")],
...
async def cb_queue_clear(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text("⚠️ Удалить очередь и всю историю?", reply_markup=InlineKeyboardMarkup(kb))
...
async def cb_queue_view(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not rows:
        ...
        await q.edit_message_text("📭 Очередь пуста.", reply_markup=InlineKeyboardMarkup(kb)); return
    kb = [[InlineKeyboardButton(f"{s_emoji(r[2])} {r[1]} — {s_name(r[2])}", callback_data=f"qitem_{r[0]}")] for r in rows]
...
async def cb_queue_item(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not row:
        await q.edit_message_text("❌ Номер не найден."); return
...
async def cb_adm_ban(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text("⚡ *Бан/Разбан* ⚡\n\nВыберите действие:",
                              parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(kb))
...
async def cb_unban_do(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = [[InlineKeyboardButton("⬅️ Назад", callback_data="adm_ban")]]
    await q.edit_message_text("✅ *Разбан*\n\nОтправьте `@username` или `ID`:",
                              parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(kb))
...
async def _edit_settings(edit_fn):
    cfg = {k: get_setting(k) for k in ("channel_url","support_id","sub_url","sub_id","tariff","hold","accept_sms","accept_qr")}
...
async def cb_setting_edit(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text(f"⚙️ *Изменение*\n\n{prompt}", parse_mode="Markdown",
                              reply_markup=InlineKeyboardMarkup(kb))
    ...
    with db() as conn:
        rows = conn.execute(
            """SELECT an.id, an.number, an.user_id, an.username, an.stood_at,
                      an.status as an_status,
                      COALESCE(dr.paid, 0) as paid,
               ...
               WHERE an.status IN ('stood','paid')
               AND DATE(an.stood_at)=?""",
            (today, today)
        ).fetchall()
        total_stood = conn.execute(
            "SELECT COUNT(*) FROM active_numbers WHERE status IN ('stood','paid','fell') AND DATE(stood_at)=?",
            (today,)
        ).fetchone()[0]

    seen = set()
    paid_cnt = hold_cnt = expired_cnt = unpaid_cnt = 0
    pending_list = []

    for r in rows:
        an_id, number, uid, uname, stood_at_str, an_status, paid_amt, pay_status = r
        key = (an_id,)
        if key in seen:
            continue
        seen.add(key)

        # Уже выплачено если: an.status='paid' ИЛИ dr.paid>0 ИЛИ pay_status in (paid,pending)
        already_paid = (
            an_status == 'paid'
            or (paid_amt and float(paid_amt) > 0)
            or pay_status in ('paid', 'pending')
        )
        if already_paid:
            paid_cnt += 1
            continue
        ...
        if elapsed < hold_sec:
            ...
            pending_list.append((dr_id, number, uid, uname, tariff))
...
async def crypto_create_check(amount: float, currency: str = "USDT") -> dict:
    """Создаёт чек CryptoPay. Если метод отключён — фолбэк на invoice."""
    headers = {"Crypto-Pay-API-Token": CRYPTO_PAY_TOKEN}
    ...
    try:
        async with aiohttp.ClientSession() as s:
            async with s.post(f"{CRYPTO_PAY_API}/createCheck",
                ...
                err = data.get("error", {})
                if err.get("name") == "METHOD_DISABLED":
                    log.warning("createCheck DISABLED — переключаюсь на createInvoice")
                else:
                    log.error(f"createCheck FAILED: {err}")
                    return {}
    except Exception as e:
        log.error(f"createCheck exception: {e}")
        return {}

    # Фолбэк: createInvoice
    payload2 = {
        "asset": currency,
        "amount": f"{amount:.2f}",
        "description": "Выплата",
    ...
    try:
        async with aiohttp.ClientSession() as s:
            async with s.post(f"{CRYPTO_PAY_API}/createInvoice",
                              json=payload2, headers=headers) as r:
                data = await r.json()
                log.info(f"createInvoice fallback response: {data}")
                if data.get("ok"):
                    result = data["result"]
                    log.info(f"createInvoice OK, fields: {list(result.keys())}")
                    return {"type": "invoice", **result}
                log.error(f"createInvoice FAILED: {data.get('error')}")
                return {}
    except Exception as e:
        log.error(f"createInvoice exception: {e}")
        return {}
...
async def _send_check_to_user(context, uid: int, number: str, amount: float) -> bool:
    ...
    btn_label = "🎁 Получить чек" if pay_type == "check" else "💳 Получить выплату"
    kb = [[InlineKeyboardButton(btn_label, url=check_url)]]
    try:
        await context.bot.send_message(
            uid,
            f"🎁 *Выплата*\n\n"
...
async def _edit_payments(edit_fn):
    ...
    today = date.today().isoformat()
    with db() as conn:
        fell_cnt = conn.execute(
            "SELECT COUNT(*) FROM active_numbers WHERE status='fell' AND DATE(stood_at)=?",
            (today,)
        ).fetchone()[0]

    text = (
        "⚡ *Выплаты* ⚡\n\n"
        f"Тариф: *${s['tariff']:.2f}* за номер\n"
        f"Холд: *{s['hold_sec'] // 60}* мин\n\n"
        f"Сегодня встало: *{s['stood']}*\n"
        f"Сегодня слетело: *{fell_cnt}*\n"
        f"Сегодня отстояло холд: *{s['expired']}*\n"
        f"Оплачено: *{s['paid']}*\n"
        f"Не оплачено: *{s['unpaid']}*\n"
        f"В холде: *{s['in_hold']}*\n"
    )

    # Стак ожидающих выплаты — только если момент оплаты включён
    ...
    kb = [
        ...
        [InlineKeyboardButton(f"⏳ Холд выплат: {s['hold_sec'] // 60} мин", callback_data="pay_set_hold")],
...
async def cb_pay_daily(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with db() as conn:
        rows = conn.execute(
            """SELECT dr.id, dr.number, dr.user_id, dr.username,
                      ...
                      an.created_at, an.stood_at, an.status,
                      dr.paid
               FROM daily_report dr
               LEFT JOIN active_numbers an ON an.user_id=dr.user_id AND an.number=dr.number
    ...
    for dr_id, number, uid, uname, op_uname, op_id, ptype, created_at, stood_at, an_status, paid in rows:
        # В отчёт только встоявшие И слетевшие (не отменённые/не встал)
        if an_status in ("cancelled", "not_stood", "timeout", None):
            continue
        num_cnt += 1
        method    = "СМС" if ptype == "sms" else "QR" if ptype == "qr" else "—"
        status_ru = STATUS_RU.get(an_status, an_status or "—")
        ...
        # Простой (между stood_at и fell_at если слетел)
        duration = "—"
        if stood_at and an_status == "fell":
            try:
                with db() as c2:
                    fell_row = c2.execute(
                        "SELECT created_at FROM active_numbers WHERE user_id=? AND number=? AND status='fell'",
                        (uid, number)
                    ).fetchone()
                if fell_row and fell_row[0]:
                    s    = datetime.fromisoformat(stood_at)
                    e    = datetime.fromisoformat(fell_row[0])
                    diff = e - s
                    h, rem = divmod(int(diff.total_seconds()), 3600)
                    m, sc  = divmod(rem, 60)
                    duration = f"{h}:{m:02d}:{sc:02d}"
            except: pass

        block = [
            f"#{dr_id:<6} Номер: {number}",
            f"  Метод: {method}",
            f"  Статус: {status_ru}",
            f"  Дроп: @{uname or '—'} (ID: {uid})",
            f"  Оператор: @{op_uname or '—'} (ID: {op_id or '—'})",
            f"  Сдан: {fmt_t(created_at)}",
            f"  Встал: {fmt_t(stood_at)}",
            f"  Простоял: {duration}",
            "-" * 40,
            "",
        ]
...
async def cb_pay_del_report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not rows:
        await q.edit_message_text("📊 Отчёт пуст.",
                                  reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⬅️ Назад", callback_data="adm_payments")]]))
        return
    ...
    await q.edit_message_text("🗑 *Удалить номер из отчёта:*",
                              parse_mode="Markdown", reply_markup=InlineKeyboardMarkup(kb))
...
async def cb_user_pay(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not invoice:
        await q.edit_message_text("❌ Ошибка создания счёта. Попробуйте позже.")
        return
...
def main():
    ...
    msg_user_conv = ConversationHandler(
        ...
        states={
            WAIT_MSG_TARGET: [MessageHandler(filters.TEXT & ~filters.COMMAND, wait_msg_target)],
            WAIT_MSG_TEXT:   [MessageHandler(filters.TEXT & ~filters.COMMAND, wait_msg_text)],
        },
    ...
    broadcast_conv = ConversationHandler(
        ...
        states={WAIT_BROADCAST: [MessageHandler(filters.TEXT & ~filters.COMMAND, wait_broadcast)]},
        fallbacks=[CallbackQueryHandler(cb_back_admin, pattern="^back_admin$")],
        per_message=False, per_chat=False, per_user=True,
    )
    ...
    ban_conv = ConversationHandler(
        ...
        states={
            WAIT_BAN_TARGET:   [MessageHandler(filters.TEXT & ~filters.COMMAND, wait_ban_target)],
            WAIT_UNBAN_TARGET: [MessageHandler(filters.TEXT & ~filters.COMMAND, wait_ban_target)],
        },
    ...
    settings_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(cb_setting_edit,
                      pattern="^(set_ch_url|set_sup_id|set_sub_url|set_sub_id|set_tariff|set_hold|pay_set_hold)$")],
        states={WAIT_SETTING_VALUE: [MessageHandler(filters.TEXT & ~filters.COMMAND, wait_setting_value)]},
    ...
    # Уведомление об истории каждый день в 00:05
    from datetime import time as dtime
    app.job_queue.run_daily(daily_history_notify, time=dtime(0, 5, 0))
    ...
    app.add_handler(CallbackQueryHandler(cb_phone_type,       pattern="^phone_type_(sms|qr)$"))
