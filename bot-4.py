...
async def handle_qr_photo_from_op(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if not reply or reply.from_user is None or not reply.from_user.is_bot:
        return False
...
async def _main_menu_data():
    ...
    text   = (
        f"⚡ *Главное меню* ⚡\n\n"
        f"⏳ Очередь: *{cnt}* номеров\n\n"
        f"{status}\n"
        f"└ Прайс: *{tariff}$* за номер"
    )
...
async def cb_pay_daily(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for row in rows:
        ...
        # Пропускаем отменённые/не вставшие
        if display_status in ("cancelled", "not_stood", "timeout"):
            continue
        if not number:
            continue
        num_cnt += 1
        ...
        def _calc_duration(start_str, end_str=None):
            ...
            try:
                ...
                m2, sc = divmod(rem, 60)
                return f"{h}:{m2:02d}:{sc:02d}" if h else f"{m2}м {sc}с"
            except:
                return "—"
        ...
        if display_status == "fell":
            # Простоял от stood_at до fell_at
            duration = _calc_duration(stood_at, fell_at)
            if duration == "—" and fell_at:
                # fell_at может быть в active_numbers
                with db() as c2:
                    fell_row = c2.execute(
                ...
                if fell_row and fell_row[0]:
                    duration = _calc_duration(stood_at, fell_row[0])
        elif display_status in ("stood", "paid"):
            # Стоит до сих пор — считаем от stood_at до сейчас
