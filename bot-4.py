...
SMS_CODE_TIMEOUT = 60   # секунд на ввод СМС кода
...
async def handle_sms_code(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    number, uname, chat_id, topic_id, group_msg_id, _ = row
    uname_safe = esc_md(uname) if uname else str(uid)
    is_buyer_chat = (chat_id > 0)
    ...
    try:
        ...
        if topic_id:
            send_kw["message_thread_id"] = topic_id
        if group_msg_id and not is_buyer_chat:
            # В группе — отвечаем на карточку
            send_kw["reply_to_message_id"] = group_msg_id
        await context.bot.send_message(**send_kw)

        # В личке покупателя удаляем старую карточку "Номер взят"
