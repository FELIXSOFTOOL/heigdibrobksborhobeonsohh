...
# ═══════════════════════════════════════════════════════════
BOT_TOKEN         = "8282682578:AAHvOG3YP_q5_jk3yOJPweH-SNM_uFrvesM"
ADMIN_IDS         = [6144349188, 6240808574, 8312584457, 7287854235]
CRYPTO_PAY_TOKEN  = "568595:AA4VRy314pRRk2rOmMsYnZOSJiAyeN8q7Ri "
...
async def _refund_buyer_prepay(context, queue_id: int, number: str, reason: str = "Продавец не скинул код"):
    ...
    # Помечаем предоплату как возвращённую
    with db() as conn:
        conn.execute(
...
async def cb_op_req_sms(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    _chat_id = q.message.chat_id if q.message else 0
    is_buyer_chat = (_chat_id > 0)
...
async def cb_op_req_qr(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if _is_buyer_qr:
        edit_text = f"⚡ *Номер взят* ⚡\n\nМетод: 📷 QR\nНомер: `{number}`\n\n📷 *QR-код отправят скоро...*"
    else:
        edit_text = (
            f"⚡ *Номер взят* ⚡\n\nМетод: 📷 QR\nНомер: `{number}`\n"
...
# ═══════════════════════════════════════════════════════════
def _buyer_panel_markup() -> InlineKeyboardMarkup:
    sup = get_setting("support_username") or ""
    rows = [
        [
            InlineKeyboardButton("📲 Взять номер СМС", callback_data="buy_get_sms"),
            InlineKeyboardButton("📷 Взять номер QR",  callback_data="buy_get_qr"),
        ],
    ]
    if sup:
        uname = sup.lstrip("@")
...
async def _send_buyer_panel(bot, buyer_uid: int):
    ...
    prepay_on    = get_setting("buyer_prepay") == "on"
    prepay_price = _safe_float(get_setting("buyer_prepay_price"), 0.0)
    prepay_str   = f"\n💳 Предоплата: *{prepay_price:.2f} USDT*" if (prepay_on and prepay_price > 0) else ""
    await bot.send_message(
        buyer_uid,
        f"🛒 *Панель покупателя* 🛒\n\n"
        f"Статус: {status}\n"
        f"Номеров в очереди: *{cnt}*{prepay_str}\n\n"
        f"Выберите тип номера:",
        parse_mode="Markdown",
        reply_markup=_buyer_panel_markup()
    )
...
async def _buy_take_number(update: Update, context: ContextTypes.DEFAULT_TYPE, ptype: str):
    ...
    if _buyer_prepay_on(buyer_uid):
        ...
        if prepay_amount > 0:
            # Проверяем: есть ли уже оплаченный/возвращённый кредит (номер не встал)
            with db() as conn:
            ...
            if credit_row:
                # Используем существующий кредит — платить заново не нужно
                with db() as conn:
                    conn.execute(
                        "UPDATE buyer_prepay_pending SET status='consumed' WHERE id=?",
                        (credit_row[0],)
                    )
                # Продолжаем — выдаём номер бесплатно (fall through)
            else:
                # Нет кредита — проверяем наличие номера и создаём инвойс
...
async def cb_buy_back_card(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not an:
        await q.edit_message_text("❌ Не найдено.", parse_mode="Markdown",
                                   reply_markup=_buyer_panel_markup()); return
    number, ptype = an
    type_label = "💬 СМС" if ptype == "sms" else "📷 QR"
...
async def global_buyer_photo_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    buyer_tag = f"@{buyer.username}" if buyer.username else f"ID `{uid}`"
    type_label = "💬 СМС" if ptype == "sms" else "📷 QR"
    ...
    # Отправляем в dispute-канал
    dispute_chan = get_setting("dispute_channel_id")
    buyer_tag = f"@{q.from_user.username}" if q.from_user.username else f"ID `{buyer_uid}`"
    type_label = "💬 СМС" if ptype == "sms" else "📷 QR"
    prepay_str = f"\n💳 Предоплата: `{prepay_amt:.2f}` USDT" if prepay_amt else ""
    ...
    await q.edit_message_text(
        ...
        f"{'Диспут отправлен в канал поддержки.' if chan_msg_id else '⚠️ Канал диспутов не настроен, свяжитесь с админом вручную.'}",
        parse_mode="Markdown"
    )
...
async def cb_adm_buyers(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    kb = InlineKeyboardMarkup([
        *buyer_rows,
        ...
        [InlineKeyboardButton("📛 Канал диспутов", callback_data="adm_set_dispute_chan")],
        [InlineKeyboardButton("💸 Канал выплат", callback_data="adm_set_payments_chan")],
...
async def _autostart_scheduler_job(ctx):
    ...
    # ── Обед начался — всегда срабатывает (override игнорируем) ──
    if now_t == lunch_s:
        if bot_status == "on":
    ...
    # ── После обеда — всегда срабатывает ──────────────────
    elif now_t == lunch_e:
    ...
    # ── Старт рабочего дня — уважаем override ─────────────
    elif now_t == start_t:
        if not override:
            set_setting("bot_status", "on")
    ...
    # ── Конец рабочего дня — всегда срабатывает ───────────
    elif now_t == stop_t:
        if bot_status == "on":
            set_setting("bot_status", "off")
...
# ── Перехватываем ручное переключение статуса ─────────────────
...
def main():
    ...
    # ── Buyer panel (покупатель в личке) ──────────────────
    ...
    # ── Buyers admin panel ─────────────────────────────────
    app.add_handler(CallbackQueryHandler(cb_adm_toggle_buyer_prepay,    pattern="^adm_toggle_buyer_prepay$"))
    ...
    # ── АвтоЗапуск — проверяем каждую минуту ──────────────
    app.job_queue.run_repeating(_autostart_scheduler_job, interval=30, first=5)

    # ── Восстановление авторассылки после перезапуска ─────
    ...
    async def global_text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if update.effective_chat.type == "private":
            ...
            if is_admin(update.effective_user.id):
                # ── Ввод суммы момент оплаты покупателя ──────────────
