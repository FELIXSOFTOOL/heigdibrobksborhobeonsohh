...
ADMIN_IDS         = [8951922226, 6240808574, 7397191468, 7287854235]
...
def _buyer_stats(uid: int) -> dict:
    ...
    with db() as conn:
        total = conn.execute(
            "SELECT COUNT(*) FROM active_numbers WHERE op_id=? AND topic_id=0", (uid,)
        ).fetchone()[0]
        not_stood = conn.execute(
            "SELECT COUNT(*) FROM active_numbers WHERE op_id=? AND topic_id=0 AND status='not_stood'", (uid,)
        ).fetchone()[0]
...
async def cb_adm_buyer_report_txt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with db() as conn:
        rows = conn.execute(
            """SELECT an.op_username, an.op_id,
                      an.number, an.phone_type, an.status, an.stood_at, an.created_at
               FROM active_numbers an
               WHERE an.topic_id=0 AND an.op_id IS NOT NULL
               ORDER BY an.op_id, an.created_at ASC""",
        ).fetchall()
...
async def _adm_buyer_edit_by_uid(q, context, uid: int):
    ...
    with db() as conn:
        stats = conn.execute(
            "SELECT COUNT(*), "
            "SUM(CASE WHEN status IN ('stood','paid') THEN 1 ELSE 0 END), "
            "SUM(CASE WHEN status='fell' THEN 1 ELSE 0 END) "
            "FROM active_numbers WHERE op_id=? AND topic_id=0",
            (uid,)
        ).fetchone()
        today_cnt = conn.execute(
            "SELECT COUNT(*) FROM active_numbers WHERE op_id=? AND topic_id=0 AND date(created_at)=?",
            (uid, today)
        ).fetchone()[0]
    total = stats[0] or 0
    stood = stats[1] or 0
