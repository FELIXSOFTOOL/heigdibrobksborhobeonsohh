...
async def handle_phone_number(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    #    Проверяем daily_report, queue и active_numbers (любой статус за сегодня)
    with db() as conn:
        ...
        if not already_today:
            already_today = conn.execute(
                "SELECT id FROM queue WHERE number=? AND DATE(loaded_at)=?",
                (cleaned, today_iso)
            ).fetchone()
        if not already_today:
            already_today = conn.execute(
                "SELECT id FROM active_numbers WHERE number=? AND DATE(created_at)=?",
                (cleaned, today_iso)
            ).fetchone()
    if already_today:
        await update.message.reply_text(
...
def _payments_stats() -> dict:
    ...
    with db() as conn:
        # Встало = номер у которого stood_at за сегодня (исключаем очищенные)
        stood_cnt = conn.execute(
            "SELECT COUNT(*) FROM active_numbers WHERE stood_at LIKE ? AND status NOT IN ('report_cleared','cancelled')",
            (today_prefix,)
        ).fetchone()[0]
        ...
        # Слетело до холда = status='cancelled' И stood_at задан
