...
ADMIN_IDS         = [914120031, 6240808574, 8312584457, 7287854235]
...
def init_db():
    ...
    migrations = [
        ...
        ("users",          "is_buyer",        "ALTER TABLE users ADD COLUMN is_buyer INTEGER DEFAULT 0"),
    ]
...
def is_buyer(uid: int) -> bool:
    """Проверяет, выдана ли пользователю роль покупателя."""
    with db() as conn:
        row = conn.execute(
            "SELECT is_buyer FROM users WHERE user_id=?", (uid,)
        ).fetchone()
    return bool(row and row[0])


def grant_buyer(uid: int):
    """Выдаёт пользователю роль покупателя."""
    with db() as conn:
        conn.execute(
            "INSERT OR IGNORE INTO users (user_id, is_buyer, registered) VALUES (?,1,?)",
            (uid, now_msk().isoformat())
        )
        conn.execute("UPDATE users SET is_buyer=1 WHERE user_id=?", (uid,))


def revoke_buyer(uid: int):
    """Забирает роль покупателя."""
    with db() as conn:
        conn.execute("UPDATE users SET is_buyer=0 WHERE user_id=?", (uid,))
...
async def cmd_paymax(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """/paymax — покупатель сам открывает панель. Админ с ID — выдаёт роль."""
...
async def _buy_take_number(update: Update, context: ContextTypes.DEFAULT_TYPE, ptype: str):
    ...
    # ── Предоплата ────────────────────────────────────────────
    if get_setting("buyer_prepay") == "on":
        prepay_amount = _safe_float(get_setting("buyer_prepay_price"), 0.0)
        if prepay_amount > 0:
    ...
    if ptype == "sms":
        card_kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("💬 Запросить СМС",  callback_data=f"op_req_sms_{queue_id}"),
             InlineKeyboardButton("❌ Отменить номер", callback_data=f"op_cancel_{queue_id}")],
            [InlineKeyboardButton("🚫 Номер не встал", callback_data=f"buy_dispute_{queue_id}")],
        ])
    else:
        card_kb = InlineKeyboardMarkup([
...
# ═══════════════════════════════════════════════════════════
#  🚫  BUYER DISPUTE — номер не встал
# ═══════════════════════════════════════════════════════════
async def cb_buy_dispute(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Покупатель нажал '🚫 Номер не встал' — создаём диспут в support-канале."""
    q = update.callback_query
    await q.answer()
    buyer_uid = q.from_user.id
    queue_id  = int(q.data.split("buy_dispute_", 1)[1])
...
async def cb_adm_buyers(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with db() as conn:
        rows = conn.execute(
            """SELECT op_username, op_id,
                      COUNT(*) as total,
                      SUM(CASE WHEN phone_type='sms' THEN 1 ELSE 0 END) as sms_cnt,
                      SUM(CASE WHEN phone_type='qr'  THEN 1 ELSE 0 END) as qr_cnt
               FROM active_numbers
               WHERE topic_id=0 AND op_id IS NOT NULL
               GROUP BY op_id
               ORDER BY total DESC""",
        ).fetchall()

        today_rows = conn.execute(
            """SELECT op_username, op_id, COUNT(*) as today
               FROM active_numbers
               WHERE topic_id=0 AND op_id IS NOT NULL
                 AND date(created_at) = ?
               GROUP BY op_id""",
        ...
        # Слетело ДО холда (bought and then fell early) — по каждому покупателю
        fell_rows = conn.execute(
            """SELECT op_id, COUNT(*) as fell
               FROM active_numbers
               WHERE topic_id=0 AND op_id IS NOT NULL
                 AND status='cancelled' AND stood_at IS NOT NULL
                 AND stood_at LIKE ?
               GROUP BY op_id""",
            (today_prefix,)
        ).fetchall()

    today_map = {r[1]: r[2] for r in today_rows}
    fell_map  = {r[0]: r[1] for r in fell_rows}

    if not rows:
        text = "🛒 *Статистика покупателей*\n\nПокупателей пока нет."
    else:
        lines = ["🛒 *Статистика покупателей*\n"]
        for i, r in enumerate(rows, 1):
            uname, uid, total, sms, qr = r
            name  = f"@{esc_md(uname)}" if uname else f"ID `{uid}`"
            today_cnt = today_map.get(uid, 0)
            fell_cnt  = fell_map.get(uid, 0)
            lines.append(
                f"{i}. {name}\n"
                f"   Всего: *{total}* | Сегодня: *{today_cnt}*\n"
                f"   СМС: {sms} | QR: {qr} | ❌ До холда: {fell_cnt}"
            )
        text = "\n".join(lines)

    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("📄 Отчёт покупателей (.txt)", callback_data="adm_buyer_report_txt")],
        [InlineKeyboardButton(
            f"💳 Момент оплата: {'✅ ВКЛ' if get_setting('buyer_prepay') == 'on' else '❌ ВЫКЛ'}",
            callback_data="adm_toggle_buyer_prepay"
        )],
        [InlineKeyboardButton("💰 Сумма момент оплаты", callback_data="adm_set_buyer_prepay_price")],
...
async def cb_adm_set_dispute_chan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Устанавливает ID канала диспутов."""
    q = update.callback_query
    if not is_admin(q.from_user.id):
        await q.answer(); return
    await q.answer()
    ...
    await qedit(
        ...
        f"Бот должен быть администратором в этом канале.",
        parse_mode="Markdown",
        reply_markup=kb
    )
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(cb_buy_dispute,       pattern="^buy_dispute_"))
            ...
            if is_admin(update.effective_user.id):
                ...
                if context.user_data.get("wait_buyer_prepay_price"):
                    ...
                    try:
                        ...
                        await update.message.reply_text(
                            f"✅ Сумма момент оплаты установлена: `{val:.2f}` USDT",
                            parse_mode="Markdown"
                        )
                    except ValueError:
                ...
                if context.user_data.get("wait_dispute_channel_id"):
                    ...
                    await update.message.reply_text(
                        f"✅ Канал диспутов установлен: `{raw}`",
                        parse_mode="Markdown"
                    )
                    return
