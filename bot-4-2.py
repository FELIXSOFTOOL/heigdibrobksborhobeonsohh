...
async def cb_drop_qr_scanned(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    uname_safe_scanned = esc_md(uname) if uname else str(uid)
    try:
        send_kw = {
            "chat_id": chat_id,
            "text": (
                f"⚡ *QR отсканирован* ⚡\n\nНомер: `{number}`\n"
                f"Дроп: @{uname_safe_scanned}\n\nПодтверди результат:"
            ),
            "parse_mode": "Markdown",
            "reply_markup": InlineKeyboardMarkup([[
...
PASS_TIMEOUT = 120  # секунд на ввод пароля

async def _timeout_pass(context):
    """Таймаут ввода пароля — уведомляем обе стороны."""
    queue_id = context.job.data
    with db() as conn:
        row = conn.execute(
            "SELECT number, user_id, username, chat_id, topic_id FROM active_numbers WHERE queue_id=?",
            (queue_id,)
        ).fetchone()
    if not row:
        return
    ...
    context.bot_data.pop(f"wait_pass_msg_{uid}", None)

    try:
        await context.bot.send_message(
            uid,
            f"⏰ Время вышло. Пароль для номера `{number}` не был введён.",
            parse_mode="Markdown"
        )
    except: pass
    ...
    try:
        send_kw = {
            "chat_id": chat_id,
            "text": f"⏰ *Пароль не введён* — таймаут\n\nНомер: `{number}`\nДроп: @{uname_safe}",
            "parse_mode": "Markdown",
            "reply_markup": InlineKeyboardMarkup([[
...
async def _do_req_pass(context, q, queue_id: int, is_repeat: bool = False):
    """Общая логика запроса пароля — по паттерну СМС: сначала дропу, потом карточка."""
    with db() as conn:
        row = conn.execute(
    ...
    op_safe    = esc_md(op_uname) if op_uname else "—"
    type_label = "💬 СМС" if ptype == "sms" else "📷 QR"
    label      = "Повтор пароля" if is_repeat else "Пароль запрошен"

    # Сначала шлём дропу — если упало, не меняем карточку
    try:
        sent = await context.bot.send_message(
            uid,
            f"🗝️ *{'Повтор: введите' if is_repeat else 'Введите'} пароль от аккаунта MAX* {PASS_EMOJI}\n\n"
            f"Номер: `{number}`\n\n"
            f"*ПРИШЛИТЕ ПАРОЛЬ ОТВЕТОМ НА СООБЩЕНИЕ* 👇",
            parse_mode="Markdown"
        )
        # Храним в dict {msg_id: queue_id}
        pending = context.bot_data.setdefault(f"pass_pending_{uid}", {})
        pending[sent.message_id] = queue_id
        context.bot_data[f"wait_pass_msg_{uid}"] = sent.message_id
        _cancel_job(context, f"pass_timeout_{queue_id}")
        context.job_queue.run_once(
            _timeout_pass, PASS_TIMEOUT, data=queue_id, name=f"pass_timeout_{queue_id}"
        )
    except Exception as e:
    ...
    # Только после успешной отправки — редактируем карточку в группе
    await safe_edit(
        q.edit_message_text,
        f"⚡ *Пароль от Номера* ⚡\n\n"
        f"Метод: {type_label}\n"
...
async def _edit_settings(edit_fn):
    cfg = {k: get_setting(k) for k in ("channel_url","support_id","sub_url","sub_id","tariff","tariff_skip","tariff_qr","tariff_qr_skip","hold","accept_sms","accept_qr","pay_log_id")}
...
def main():
    ...
    settings_conv = ConversationHandler(
        entry_points=[CallbackQueryHandler(cb_setting_edit,
                      pattern="^(set_ch_url|set_sup_id|set_sub_url|set_sub_id|set_tariff|set_tariff_skip|set_tariff_qr|set_tariff_qr_skip|set_hold|pay_set_hold|set_pay_log_id)$")],
